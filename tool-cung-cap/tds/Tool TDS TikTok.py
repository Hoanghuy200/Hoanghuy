...
class TraoDoiSub_Api (object):
	...
	def main(self):
		try:
			main = requests.get('https://traodoisub.com/api/?fields=profile&access_token='+self.token, impersonate="safari184_ios").json()
			try:
				return main['data']
			except:
				False
	...
	def run(self, user):
		try:
			run = requests.get(f'https://traodoisub.com/api/?fields=tiktok_run&id={user}&access_token={self.token}', impersonate="safari184_ios").json()
			try:
				return run['data']
			except:
				return False
	...
	def get_job(self, type):
		try:
			get = requests.get(f'https://traodoisub.com/api/?fields={type}&access_token={self.token}', impersonate="safari184_ios")
			return get
		except:
			return False
...
#TIKTOK_LIKE_CACHE, TIKTOK_FOLLOW_CACHE
		try:
			cache = requests.get(f'https://traodoisub.com/api/coin/?type={type}&id={id}&access_token={self.token}', impersonate="safari184_ios").json()
			try:
				cache['cache']
				return True
			except:
	...
	def nhan_xu(self, id, type):
		try:
			nhan = requests.get(f'https://traodoisub.com/api/coin/?type={type}&id={id}&access_token={self.token}', impersonate="safari184_ios")
			try:
