...
def chonacc():
  ...
  response = requests.get('https://gateway.golike.net/api/snapchat-account', headers=headers, json=json_data,impersonate="chrome").json()
  return response
def nhannv(account_id):
  ...
  response = requests.get('https://gateway.golike.net/api/advertising/publishers/snapchat/jobs',params=params,headers=headers,json=json_data,impersonate="chrome").json()
  return response
...
chonsnapchat = chonacc()  
def dsacc():
  if(chonsnapchat["status"]!=200):
    print("\033[1;34m Authorization hoặc T sai hãy nhập lại!!! [ XIN VUI LÒNG NHẬP LẠI ATHU VÀ T Ạ]")
    quit()

  for i in range(len(chonsnapchat["data"])):
    print(f'\033[1;97m•[✩]➭\033[1;36m [{i+1}] \033[1;91m=> \033[1;97mTên Tài Khoản┊\033[1;32m㊪ :\033[1;93m {chonsnapchat["data"][i]["name"]}  ')
...
while True:
  try:
    ...
    while luachon > len((chonsnapchat)["data"]):
      luachon = int(input("\033[1;32mAcc Này Không Có Trong Danh Sách , Hãy Nhập Lại : "))
    account_id = chonsnapchat["data"][luachon - 1]["id"]
    break  
  except:
...
while True:
  if checkdoiacc == doiacc:
    dsaccloi.append(chonsnapchat["data"][luachon - 1]["nickname"])
    print(f"\033[1;36mCác Acc snapchat {dsaccloi} Có Vẻ Gặp Vấn Đề Nên Đổi Acc Chạy Đê ")
    dsacc()
    while True:
      try:
        luachon = int(input("\033[1;35m\033[1;97m║ Chọn \033[1;96mTài \033[1;95mKhoản \033[1;94mĐể \033[1;93mChạy \n\033[1;97m╚⟩⟩⟩  "))
