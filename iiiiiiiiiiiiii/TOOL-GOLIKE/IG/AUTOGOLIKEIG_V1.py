    ...
    try:
        with open('authorization.txt', 'r', encoding='utf-8') as file:
            ...
            if not authorization:
                raise ValueError("authorization.txt is empty")
        ...
        print("Error: One or both files (authorization.txt, T.txt) not found")
        return None
...
def delete_files():
    for file_path in ['authorization.txt', 'T.txt']:
        try:
...
if __name__ == "__main__":
    while True:
        ...
        if tthu:
            authorization, T = tthu
        else:
            delete_files()  # Delete files if they exist but are invalid
            banner()
            authorization = input('NHẬP authorization: ').strip()
            if not authorization:
                print("Error: Authorization cannot be empty")
                exit(1)
            if not save_to_file(authorization, 'authorization.txt'):
                print("Error: Failed to save authorization")
                exit(1)
            
            T = input('NHẬP T: ').strip()
            if not T:
                print("Error: T cannot be empty")
                exit(1)
            if not save_to_file(T, 'T.txt'):
                print("Error: Failed to save T")
                exit(1)
        ...
        headers = {
            ...
            'T': T,
            'User-Agent': "Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.0 Mobile/15E148 Safari/604.1 (iPhone 16)",
            'Authorization': authorization,
        ...
        try:
            response = requests.get(url1, headers=headers, impersonate="chrome")
            ...
            if response.status_code == 401:
                print("Error: HTTP 401 Unauthorized - Invalid authorization or T")
                delete_files()  # Delete files on 401 error
                print("Please re-enter credentials")
                continue  # Retry after re-entering
            else:
