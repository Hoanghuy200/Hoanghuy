...
class TraoDoiSub_Api (object):
	...
	def main(self):
		try:
			main = scraper.get('https://traodoisub.com/api/?fields=profile&access_token='+self.token).json()
			try:
				return main['data']
			except:
				False
	...
	def run(self, user):
		try:
			run = scraper.get(f'https://traodoisub.com/api/?fields=tiktok_run&id={user}&access_token={self.token}').json()
			try:
				return run['data']
			except:
				return False
	...
	def get_job(self, type):
		try:
			get = scraper.get(f'https://traodoisub.com/api/?fields={type}&access_token={self.token}')
			return get
		except:
			return False
...
#TIKTOK_LIKE_CACHE, TIKTOK_FOLLOW_CACHE
		try:
			cache = scraper.get(f'https://traodoisub.com/api/coin/?type={type}&id={id}&access_token={self.token}').json()
			try:
				cache['cache']
				return True
			except:
	...
	def nhan_xu(self, id, type):
		try:
			nhan = scraper.get(f'https://traodoisub.com/api/coin/?type={type}&id={id}&access_token={self.token}')
			try:
...
def delay(dl):
  try:
    for i in range(dl, -1, -1):
       print(f'{vang}[{trang}BÉ TẬP CODE{vang}][{trang}'+str(i)+vang+']           ',end='\r')
       sleep(1)
  except:
     sleep(dl)
     print(dl,end='\r')
